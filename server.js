...
// Optional shared secret sent as X-Piston-Secret header, if you've put nginx
// or similar in front of Piston requiring it (recommended when exposing
// Piston publicly, e.g. for Heroku which has no fixed outbound IP).
...
app.post("/api/run", async (req, res) => {
  ...
  try {
    const response = await axios.post(
      ...
      {
        timeout: 25000,
        headers: PISTON_SECRET ? { "X-Piston-Secret": PISTON_SECRET } : {},
      }
    );
